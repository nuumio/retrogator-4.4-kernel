...
static int rockchip_pwm_probe(struct platform_device *pdev)
{
	...
	if (set_defaults) {
		dev_err(&pdev->dev, "Setting defaults: period: %u, duty cycle: %u, enabled: %s\n", default_period, default_duty_cycle, default_enabled ? "true" : "false");
		pwm = pwm_request_from_chip((struct pwm_chip *)pc, 0, NULL);
		if (IS_ERR(pwm)) {
			dev_err(&pdev->dev, "..But failed to get pwm device");
			goto exit_ok;
		}
